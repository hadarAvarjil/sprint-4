...
export function TestDrop({ loggedInUser }) {
  const buyerOrders = useSelector((storeState) => storeState.orderModule.buyerOrders); // Correct selector
  ...
  useEffect(() => {
    async function fetchOrders() {
      if (loggedInUser && buyerOrders?.length === 0) { // Avoid re-fetching if already loaded
        try {
      ...
      <div className="user-orders-container">
        {userOrders.map((order) => {
          const getStatusElement = (status) => {
            if (status === "approved") {
              return (
                <div style={{ backgroundColor: "green", color: "white", padding: "5px", borderRadius: "4px" }}>
                  Approved
                </div>
              );
            } else if (status === "declined") {
              return (
                <div style={{ backgroundColor: "red", color: "white", padding: "5px", borderRadius: "4px" }}>
                  Declined
                </div>
              );
            } else {
              return (
                <div style={{ color: "orange", padding: "5px", borderRadius: "4px" }}>
                  Pending
                </div>
              );
            }
          ...
          return (
            <div key={order._id} className="user-orders-gig">
              <div className="orders-wrap">
                ...
                <div className="order-details">
                  <h3 style={{ color: "#404145" }}>{order.title || "Unknown Gig Title"}</h3>

                  <div className="order-details-info">
                    <p style={{ color: "#6b7280" }}>by {order.fullName}</p>
                    <div className="gig-status">{getStatusElement(order.status || "pending")}</div>
                  </div>
                </div>
              </div>
